...
import { markDiscoveryGroupsEmailed, appendGroupLeadsToCampaignSelected } from '@/lib/sales-agent/groups';
import {
...
export async function PATCH(
  ...
  // Actions that don't need a field patch first
  if (body.action === 'queue_remaining') {
    if (Object.keys(data).length) {
      await (prisma as any).saCampaign.update({ where: { id }, data });
    }
  ...
  /** Re-queue every sequence round for all leads (second pass through the full campaign). */
  if (body.action === 'resend_all_rounds') {
    if (Object.keys(data).length) {
      await (prisma as any).saCampaign.update({ where: { id }, data });
    }
    const result = await queueCampaignEmails(id, gate.userId, { forceResend: true });
    const campaign = await (prisma as any).saCampaign.findUnique({ where: { id } });
    return jsonOk({
      ...campaign,
      queueResult: result,
      message: result?.queued
        ? `Resend queued — ${result.queued} email(s) across all rounds for ${result.leadsQueued ?? 0} lead(s).`
        : 'No emails queued for resend.',
    });
  }

  /**
   * Expand flow: after finding more leads in a group, pull new email-ready leads
   * into this campaign and queue them starting at round 1 (sequence step 1+).
   * Already-queued leads are skipped via onlyMissing.
   */
  if (body.action === 'append_group_and_queue') {
    const discoveryGroupId = Number(body.discoveryGroupId);
    if (!discoveryGroupId) {
      return jsonError('discoveryGroupId is required', 400);
    }
    try {
      if (Object.keys(data).length) {
        await (prisma as any).saCampaign.update({ where: { id }, data });
      }
      const appended = await appendGroupLeadsToCampaignSelected({
        campaignId: id,
        discoveryGroupId,
      });
      const result = await queueCampaignEmails(id, gate.userId, { onlyMissing: true });
      await saLog({
        ...
        message: `Campaign ${id}: added ${appended.addedLeadIds.length} new group leads, queued ${result?.queued ?? 0}`,
        entityType: 'SaCampaign',
        entityId: id,
        userId: gate.userId,
        details: { discoveryGroupId, appended, queueResult: result },
      });
      const refreshed = await (prisma as any).saCampaign.findUnique({ where: { id } });
      return jsonOk({
        ...refreshed,
        appendResult: appended,
        queueResult: result,
        message:
          appended.addedLeadIds.length > 0
            ? `Added ${appended.addedLeadIds.length} new lead(s) from group and queued from round 1.`
            : appended.groupEmailCount === 0
              ? 'No leads with email in that group yet. Expand discovery first, then sync again.'
              : 'No new leads to add — queued any remaining from existing selection.',
      });
    } catch (err: any) {
      return jsonError(err.message || 'Failed to append group leads', 400);
    }
  }
...
async function queueCampaignEmails(
  campaignId: number,
  userId: number,
  opts?: { onlyMissing?: boolean; forceResend?: boolean }
) {
  const campaign = await (prisma as any).saCampaign.findUnique({
    where: { id: campaignId },
  ...
  const startedAt = opts?.forceResend ? new Date() : campaign.startedAt ? new Date(campaign.startedAt) : new Date();
  if (opts?.forceResend) {
    await (prisma as any).saCampaign.update({
      where: { id: campaignId },
      data: { status: 'RUNNING', startedAt, completedAt: null, lastError: null },
    });
  ...
  let skipped = 0;
  let failed = 0;
  const existingStep1 = opts?.forceResend
    ? 0
    : await (prisma as any).saSentEmail.count({
        where: {
          campaignId,
    ...
    try {
      ...
      for (const step of steps) {
        if (!opts?.forceResend) {
          ...
          const inFlight = await (prisma as any).saSentEmail.findFirst({
            where: {
              campaignId,
              companyId: lead.id,
              sequenceStep: step.step,
  ...
  const queueSummary = {
    ...
    skipped,
    failed,
    leadsQueued: queuedLeadIds.length,
