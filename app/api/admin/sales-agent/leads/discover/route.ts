...
import { suggestDiscoveryKeywords, suggestSearchDirectives } from '@/lib/sales-agent/suggest-keywords';
...
export async function POST(request: NextRequest) {
  ...
  // AI helper for Search Directives field
  if (body.action === 'suggest_directives') {
    const countries = parseList(body.countries);
    const keywords = parseList(body.keywords);
    if (!countries.length && !keywords.length) {
      return jsonError('Add at least one country or keyword');
    }
    const suggestions = await suggestSearchDirectives({
      countries,
      cities: parseList(body.cities),
      keywords,
      industry: body.industry,
      existingNotes: body.notes || body.existingNotes,
    });
    return jsonOk(suggestions);
  }
