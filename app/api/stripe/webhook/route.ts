...
async function handlePaymentSuccess(invoice: Stripe.Invoice) {
  ...
  // Marketing partner revenue share (idempotent)
  try {
    const { createCommissionFromBillingRecord } = await import('@/lib/partners');
    await createCommissionFromBillingRecord(billingRecord.id);
  } catch (partnerErr) {
    console.error('Partner commission sync failed:', partnerErr);
  }
