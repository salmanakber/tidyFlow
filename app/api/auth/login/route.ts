...
export async function POST(request: NextRequest) {
  try {
    ...
    /** 'customer' = billing portal only (no admin cookie). Default 'admin' for console. */
    ...
    if (!isPasswordValid) {
      if (user.googleId) {
        return NextResponse.json({
          success: false,
          message: 'This account uses Google. Please use Continue with Google.',
          code: 'USE_GOOGLE',
        }, { status: 401 });
      }
      return NextResponse.json({
...
const shouldSkipOTP =
  ...
  skipOtpEmails.includes(user.email.toLowerCase());
    ...
    if (portal === 'customer') {
      // Customer billing portal: never grant admin console cookie
      response.cookies.set('authToken', '', { ...cookieBase, maxAge: 0 });
    ...
    console.log('[Login] portal=', portal, 'user:', user.email, 'isHeadSuperAdmin:', user.isHeadSuperAdmin)
