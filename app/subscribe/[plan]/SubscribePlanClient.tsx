  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!json.success || !json.data?.url) {
        const suffix =
          json.loginUrl
            ? ` Sign in at ${json.loginUrl === '/account/login' ? 'the customer dashboard' : json.loginUrl} to finish checkout.`
            : '';
        throw new Error((json.message || 'Could not start checkout') + suffix);
      }
      if (json.data.token && json.data.email) {
        try {
          storeCustomerSession(json.data.token, { email: json.data.email, role: 'OWNER' }, true);
        ...
        <div className="subscribe-plan-grid">
          ...
          <section className="responsive-panel">
            ...
            <form onSubmit={onSubmit} style={{ display: 'grid', gap: 14 }}>
              ...
              {error ? (
                <p style={{ margin: '4px 0 0', color: T.rose, fontSize: 13, fontWeight: 700 }}>
                  {error}{' '}
                  <Link href="/account/login" style={{ color: T.navy, textDecoration: 'underline' }}>
                    Customer sign in
                  </Link>
                </p>
              ) : null}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: T.inkFaint, fontSize: 12, marginTop: 4 }}>
                <span style={{ flex: 1, height: 1, background: T.border || '#E6E0D6' }} />
                or
                <span style={{ flex: 1, height: 1, background: T.border || '#E6E0D6' }} />
              </div>
              <GoogleSignInButton
