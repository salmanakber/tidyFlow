...
import { Play, Pause, Plus, Pencil, Trash2, Loader2, Settings, Users, CalendarDays, LayoutDashboard, X, RotateCcw } from "lucide-react"
...
const emptyForm = {
  ...
  sourceDiscoveryGroupId: "" as string,
}
...
export default function CampaignsTab() {
  ...
  const openEdit = (c: any) => {
    ...
    setEditingId(c.id)
    let sourceDiscoveryGroupId = ""
    try {
      const cfg = c.discoveryConfig ? JSON.parse(c.discoveryConfig) : {}
      if (cfg.sourceDiscoveryGroupId) sourceDiscoveryGroupId = String(cfg.sourceDiscoveryGroupId)
    } catch {
      /* ignore */
    }
    setForm({
      ...
      selectedLeadIds,
      leadFilter: "not_sent",
      sourceDiscoveryGroupId,
    })
    if (sourceDiscoveryGroupId) setGroupFilter(sourceDiscoveryGroupId)
    setShowForm(true)
  }
  ...
  const payload = () => ({
    ...
    discoveryConfig: {
      ...
      skipDiscovery: true,
      sourceDiscoveryGroupId: form.sourceDiscoveryGroupId
        ? Number(form.sourceDiscoveryGroupId)
        : groupFilter
          ? Number(groupFilter)
          : undefined,
    },
  ...
  const appendGroupAndQueue = async (campaignId: number, discoveryGroupId?: string | number) => {
    const gid = Number(discoveryGroupId || groupFilter)
    if (!gid) {
      setMessage({
        type: "error",
        text: "Pick the lead group this campaign should sync from first.",
      })
      return
    }
    setActionId(campaignId)
    try {
      const data = await saPatch(`/campaigns/${campaignId}`, {
        action: "append_group_and_queue",
        discoveryGroupId: gid,
      })
      const added = data.appendResult?.addedLeadIds?.length ?? 0
      const queued = data.queueResult?.queued ?? 0
      setMessage({
        type: "success",
        text:
          data.message ||
          `Synced group → added ${added} new lead(s), queued ${queued} email(s) from round 1.`,
      })
      await loadCore()
    } catch (e: any) {
      setMessage({ type: "error", text: e.message })
  ...
  /** Re-queue the full email sequence for every lead (all rounds again). */
  const resendAllRounds = async (id: number) => {
    if (
      !window.confirm(
        "Resend all rounds to every lead in this campaign? New emails will be queued for each sequence step (already in-flight sends are skipped)."
      )
    ) {
      return
    }
    setActionId(id)
    try {
      const data = await saPatch(`/campaigns/${id}`, { action: "resend_all_rounds", status: "RUNNING" })
      const q = data.queueResult
      setMessage({
        type: "success",
        text:
          data.message ||
          (q?.queued
            ? `Resend queued — ${q.queued} email(s) for ${q.leadsQueued ?? 0} lead(s).`
            : "No emails queued."),
      })
      await loadCore()
    } catch (e: any) {
  ...
  const selectEntireGroup = async () => {
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        sourceDiscoveryGroupId: groupFilter,
      }))
          ...
          <div className="grid grid-cols-1 gap-3">
            {items
              ...
              .map((c) => {
                ...
                const allRoundsSent =
                  c.sequenceProgress?.headline?.includes("All") &&
                  c.sequenceProgress?.headline?.includes("rounds sent")
                const canResendAll =
                  (c.emailsSent ?? 0) > 0 &&
                  (allRoundsSent || c.status === "COMPLETED" || c.status === "PAUSED") &&
                  c.status !== "RUNNING"
                return (
                  <div
                    key={c.id}
                    ...
                    <div className="p-4 sm:p-5 flex flex-col lg:flex-row lg:items-start gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-2 lg:justify-end shrink-0">
                        ...
                        {canResendAll && (
                          <button
                            type="button"
                            className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#0B1B3B] border border-[#E3E7F0] bg-white hover:bg-[#F6F7FB] px-3 py-2 rounded-lg disabled:opacity-50"
                            disabled={actionId === c.id}
                            title="Queue every sequence round again for all leads in this campaign"
                            onClick={() => resendAllRounds(c.id)}
                          >
                            {actionId === c.id ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
                              <RotateCcw className="w-3.5 h-3.5 text-[#D98E04]" />
                            )}
                            Resend all rounds
                          </button>
                        )}
                        {c.status === "RUNNING" && (
                          <>
                            ...
                            <button
                              type="button"
                              className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#1e3a5f] border border-sky-100 bg-sky-50 hover:bg-sky-100 px-3 py-2 rounded-lg disabled:opacity-50"
                              disabled={actionId === c.id}
                              title="After expanding a lead group, pull new emails into this campaign and queue them from round 1"
                              onClick={() => {
                                let gid = groupFilter
                                try {
                                  const cfg = c.discoveryConfig ? JSON.parse(c.discoveryConfig) : {}
                                  if (cfg.sourceDiscoveryGroupId) gid = String(cfg.sourceDiscoveryGroupId)
                                } catch {
                                  /* ignore */
                                }
                                void appendGroupAndQueue(c.id, gid)
                              }}
                            >
                              {actionId === c.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Users className="w-3.5 h-3.5" />}
                              Sync new from group
                            </button>
                            <button
                              type="button"
                        ...
                        {(c.status === "DRAFT" || c.status === "PAUSED" || c.status === "COMPLETED") && (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#1e3a5f] border border-sky-100 bg-sky-50 hover:bg-sky-100 px-3 py-2 rounded-lg disabled:opacity-50"
                            disabled={actionId === c.id}
                            title="Pull newly found leads from the linked group and queue them from round 1"
                            onClick={() => {
                              let gid = groupFilter
                              try {
                                const cfg = c.discoveryConfig ? JSON.parse(c.discoveryConfig) : {}
