...
export default function LeadDiscoveryTab() {
  ...
  const [suggestingDirectives, setSuggestingDirectives] = useState(false)
  ...
  const askAI = async () => {
    ...
    try {
      ...
      if (data.keywords?.length) {
        setKeywords(Array.from(new Set([...keywords, ...data.keywords])).slice(0, 40))
      }
      if (data.cities?.length) {
        setCities(Array.from(new Set([...cities, ...data.cities])).slice(0, 60))
      }
      setMessage({
        type: "success",
  ...
  const askAIDirectives = async () => {
    if (!countries.length && !keywords.length) {
      setMessage({ type: "error", text: "Add countries or keywords first" })
      return
    }
    setSuggestingDirectives(true)
    setMessage(null)
    try {
      const data = await saPost("/leads/discover", {
      ...
      if (data.directives?.length) {
        const merged = [aiNote, ...data.directives].filter(Boolean).join(". ").replace(/\.\./g, ".")
        setAiNote(merged.slice(0, 1200))
      }
      setMessage({
        type: "success",
        text: data.rationale || `Added ${data.directives?.length || 0} directive line(s)`,
      })
    } catch (e: any) {
      setMessage({ type: "error", text: e.message })
    } finally {
      setSuggestingDirectives(false)
    }
  }
      ...
      <div className="relative overflow-hidden bg-white rounded-2xl border border-gray-200 shadow-sm">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 text-xs font-semibold text-[#0D1E36] cursor-pointer">
              <input
                ...
                className="text-[#D97706] focus:ring-[#D97706]"
              />
              Existing group (expand — find more emails in the same batch)
            </label>
          </div>
          {groupSaveMode === "existing" && (
            <p className="text-[11px] text-slate-600 leading-relaxed">
              New leads are added to the group (duplicates skipped). Emails are harvested from websites during discovery.
              To email only the new ones: open Outreach → your campaign → <strong>Sync new from group</strong> (queues from round 1).
            </p>
          )}
          {groupSaveMode === "new" ? (
            <input
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <TagInput
            ...
            hint="AI Suggest can add up to 60 cities — more cities = more search chunks & leads"
            tags={cities}
            onChange={setCities}
        ...
        <TagInput
          ...
          hint="AI Suggest adds up to 40 keywords; more keywords × cities × countries = more leads"
          tags={keywords}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="sm:col-span-3 space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <label className="block text-xs font-bold text-[#0D1E36] uppercase tracking-wider">
                AI Search Directives (Optional)
              </label>
              <button
                type="button"
                className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#0D1E36] border border-gray-200 bg-white hover:bg-slate-50 px-2.5 py-1 rounded-lg disabled:opacity-50"
                disabled={suggestingDirectives || suggesting || discovering}
                onClick={askAIDirectives}
              >
                <Sparkles className="w-3.5 h-3.5 text-[#D97706]" />
                {suggestingDirectives ? "Writing…" : "AI helper"}
              </button>
            </div>
            <textarea
              className={`${inputCls} focus:border-[#D97706] text-sm min-h-[72px] resize-y`}
              value={aiNote}
              onChange={(e) => setAiNote(e.target.value)}
              placeholder="e.g. Focus on commercial / multi-site operators. Prefer companies with a contact page…"
              rows={2}
            />
            <p className="text-[11px] text-gray-400">
              Guides AI keyword suggestions. Use AI helper to draft lines aligned with your keywords & countries.
            </p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-gray-400 leading-snug">
              Max new leads saved <strong>per search chunk</strong> (each keyword × city × country × source).
              Google Business caps at <strong>20</strong>/chunk (API limit). Search engine up to <strong>100</strong>.
              Total ≈ chunks × yield.
            </p>
          </div>
        </div>

        {(suggesting || suggestingDirectives) && (
          <ProgressBar
            label={suggestingDirectives ? "AI writing search directives…" : "AI suggesting keywords…"}
            indeterminate
            tone="navy"
          />
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-100">
          ...
          <p className="w-full text-xs text-gray-400 leading-relaxed mt-1">
            Leads go into the group you chose (new or existing). With Search engine, we target business websites,
            crawl contact pages & footers for email, and pick up emails shown in search snippets. Duplicates and
            already-registered TidyFlow emails are skipped. After expanding a group, use Campaign → Sync new from group.
          </p>
        </div>
        </div>
      </div>
