...
import { isRegisteredPlatformEmail } from './discovery-filters';
...
export async function analyzeLeadCompany(companyId: number): Promise<any> {
  ...
  const existingEmail = String(company.email || '').trim().toLowerCase() || null;
  const crawl = await crawlWebsite(company.website);

  let primaryEmail: string | null = existingEmail;
  let picked: { primaryEmail: string | null; validEmails: string[] } = {
    primaryEmail: existingEmail,
    validEmails: existingEmail ? [existingEmail] : [],
  };

  // Email should already be found at discovery. Only run AI email-pick when missing.
  if (!existingEmail) {
    ...
    picked = await pickValidEmailWithAi({
      companyName: company.name,
      website: company.website,
      emails: emailCandidates,
      companyId,
    });
    primaryEmail = picked.primaryEmail;
  } else {
    // Keep discovery email; optionally add other crawl hits as secondary contacts later
    const extras = uniqueEmails([...(crawl.allEmailsFound || []), ...crawl.emails]).filter(
      (e) => e !== existingEmail
    );
    picked = { primaryEmail: existingEmail, validEmails: [existingEmail, ...extras].slice(0, 8) };
  }
  ...
  // Skip / remove leads whose contact email is already a registered TidyFlow account
  if (primaryEmail && (await isRegisteredPlatformEmail(primaryEmail))) {
    await saLog({
      level: 'info',
      category: 'ai',
      action: 'analyze_skipped_registered_email',
      message: `Skipped ${company.name} — email already registered on platform (${primaryEmail})`,
      entityType: 'SaLeadCompany',
      entityId: companyId,
      details: { email: primaryEmail, website: company.website },
    });
    ...
    return {
      ...
      message: 'Email already belongs to a registered TidyFlow account. Lead removed.',
    };
  }

  // Also drop if any alternate candidate is a registered login (business already on platform)
  for (const alt of picked.validEmails || []) {
    if (await isRegisteredPlatformEmail(alt)) {
      await saLog({
        level: 'info',
        category: 'ai',
        action: 'analyze_skipped_registered_email',
        message: `Skipped ${company.name} — related email already registered (${alt})`,
        entityType: 'SaLeadCompany',
        entityId: companyId,
        details: { email: alt, website: company.website },
      });
      await (prisma as any).saLeadCompany.delete({ where: { id: companyId } });
      return {
        companyId,
        deleted: true,
        reason: 'already_registered',
        message: 'A found email already belongs to a registered TidyFlow account. Lead removed.',
      };
    }
  }
