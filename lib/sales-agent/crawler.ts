...
export async function crawlWebsite(websiteUrl: string): Promise<CrawlResult> {
  ...
  if (!contactHtml) {
    for (const path of ['/contact', '/contact-us', '/contactus', '/get-in-touch', '/enquiry', '/kontakt', '/contact.html', '/about/contact']) {
      try {
  ...
  // Used to tag which services appear on the crawled page (for templates / AI).
...
/**
 * Lightweight email harvest at discovery time (no AI).
 * Prefer ranked business emails from contact / footer / homepage.
 */
export async function harvestLeadEmailFromWebsite(websiteUrl: string): Promise<{
  email: string | null;
  phones: string[];
  allEmails: string[];
  crawlOk: boolean;
  ...
  const email =
    (crawl.emails[0] || crawl.allEmailsFound[0] || null)?.toLowerCase() || null;
  return {
    email,
    phones: crawl.phones || [],
    allEmails: crawl.allEmailsFound || [],
    crawlOk: crawl.success,
    title: crawl.title,
  };
}
