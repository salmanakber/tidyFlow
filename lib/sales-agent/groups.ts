...
/** Lead IDs in a group that already have a contactable email (ready for campaigns). */
export async function getGroupLeadIdsWithEmail(groupId: number): Promise<number[]> {
  const ids = await getLeadIdsInGroup(groupId);
...
/**
 * Merge group leads (with email) into a campaign's selectedLeadIds.
 * Returns which IDs were newly added (useful for round-1 queue of only new leads).
 */
export async function appendGroupLeadsToCampaignSelected(opts: {
  campaignId: number;
  ...
  const group = await (prisma as any).saDiscoveryGroup.findUnique({
    where: { id: opts.discoveryGroupId },
    select: { id: true },
  });
  if (!group) throw new Error('Discovery group not found');

  const withEmail = await getGroupLeadIdsWithEmail(opts.discoveryGroupId);
  let cfg: Record<string, unknown> = {};
  try {
    cfg =
      typeof campaign.discoveryConfig === 'string'
        ? JSON.parse(campaign.discoveryConfig || '{}')
        : campaign.discoveryConfig || {};
  } catch {
    cfg = {};
  }

  const existing = Array.isArray(cfg.selectedLeadIds)
    ? (cfg.selectedLeadIds as unknown[]).map(Number).filter(Boolean)
    : [];
  const existingSet = new Set(existing);
  const addedLeadIds = withEmail.filter((id) => !existingSet.has(id));
  const selectedLeadIds = Array.from(new Set([...existing, ...withEmail]));

  const nextCfg = {
    ...cfg,
    audience: cfg.audience || 'selected_leads',
    skipDiscovery: true,
    selectedLeadIds,
    sourceDiscoveryGroupId: opts.discoveryGroupId,
  };

  await (prisma as any).saCampaign.update({
    where: { id: opts.campaignId },
    data: { discoveryConfig: JSON.stringify(nextCfg) },
  });

  return {
    selectedLeadIds,
    addedLeadIds,
    alreadySelected: existing.length,
    groupEmailCount: withEmail.length,
  };
}
