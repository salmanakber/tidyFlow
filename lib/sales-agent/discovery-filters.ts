...
/** True if this email belongs to an already-registered TidyFlow user/account. */
export async function isRegisteredPlatformEmail(email?: string | null): Promise<boolean> {
  ...
  if (user) return true;
  const inv = await prisma.companyInvoiceSettings
    .findFirst({
      where: { email: { equals: em, mode: 'insensitive' } },
      select: { id: true },
    })
...
export function buildSearchQueries(input: {
  ...
  if (input.profileOnly) {
    const variants = [
      ...
      `${base} office cleaning site:maps.google.com`,
    ];
    return [variants[idx % variants.length]];
  }

  const neg = '-yelp -yellowpages -angi -homeadvisor -thumbtack -bbb.org -facebook -linkedin -wikipedia -indeed';
  const expansions = [
    `${base} cleaning company contact email ${neg}`,
    `${base} commercial cleaning services website ${neg}`,
    `${base} office cleaning janitorial ${neg}`,
    `${base} janitorial company contact us ${neg}`,
    `${base} cleaning services "@" email ${neg}`,
    `"${input.keyword}" ${loc} site:.com contact ${neg}`,
    `${base} facilities cleaning company ${neg}`,
    `${base} carpet cleaning commercial ${neg}`,
  ];
  return [expansions[idx % expansions.length]];
}

/** All query variants for a location — used to maximize search-engine yield per chunk. */
export function buildAllSearchQueries(input: {
  keyword: string;
  city?: string;
  state?: string;
  country?: string;
  ...
  const count = input.profileOnly ? 4 : 8;
  const out: string[] = [];
  for (let i = 0; i < count; i++) {
    out.push(...buildSearchQueries({ ...input, chunkIndex: i }));
  }
...
export async function loadLeadFingerprints(opts?: {
  ...
  const [rows, registeredUsers, invoiceSettings] = await Promise.all([
    ...
    // Already-registered TidyFlow tenant logins — never outreach these businesses again
    prisma.user.findMany({
      where: { email: { not: '' } },
      select: { email: true },
      take: 20000,
    }),
    prisma.companyInvoiceSettings
      .findMany({
        where: { OR: [{ email: { not: null } }, { website: { not: null } }] },
        select: { email: true, website: true },
        take: 5000,
      })
      .catch(() => [] as Array<{ email: string | null; website: string | null }>),
  ]);
  ...
  for (const inv of invoiceSettings || []) {
    const em = normalizeEmail(inv?.email);
    if (em) emails.add(em);
    if (inv?.website) {
      try {
        const host = normalizeWebsite(String(inv.website));
