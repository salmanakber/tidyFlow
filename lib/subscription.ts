...
export function isCompanySubscriptionEntitled(company: {
  ...
  const trialEnds =
    company.trialEndsAt != null ? new Date(company.trialEndsAt) : null;
  const trialStillValid = !!trialEnds && trialEnds > new Date();
...
export async function requireActiveSubscription(tokenUser: {
  ...
  const active = isCompanySubscriptionEntitled(company);
...
export async function getPlanUsageSnapshot(companyId: number): Promise<PlanUsageSnapshot | null> {
  ...
  const subscriptionActive = isCompanySubscriptionEntitled(company);
