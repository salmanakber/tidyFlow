...
export async function getAllStripeSettingsForAdmin() {
  ...
  let secretMode: 'test' | 'live' | 'unknown' | '' = '';
  for (const row of rows) {
    if (row.key === STRIPE_SETTING_KEYS.secretKey && row.value) {
      try {
        const raw = row.isEncrypted ? decrypt(row.value) : row.value;
        if (raw.startsWith('sk_test_') || raw.startsWith('rk_test_')) secretMode = 'test';
        else if (raw.startsWith('sk_live_') || raw.startsWith('rk_live_')) secretMode = 'live';
        else secretMode = 'unknown';
      } catch {
        secretMode = 'unknown';
      }
    }
